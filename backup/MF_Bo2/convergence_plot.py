...
def plot_fidelity_function(obj, train_x, train_y, model: Model, likelihood: GaussianLikelihood, fidelity, output_dir, run):
    """Plot the Forrester function, observed points, and GP predictions for a given fidelity."""
    plt.figure(figsize=(10, 6))
    ...
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.title(f'Forrester Function with GP Fit (Run {run + 1}, {fidelity.capitalize()} Fidelity)')
    plt.grid(True)
    plt.legend()
    
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, f'forrester_fit_run_{run + 1}_{fidelity.lower()}.png'))
    plt.close()