...
class Forrester:
    ...
    def __init__(self, fidelity="high"):
        ...
        # Define noise levels for different fidelities
        self.noise_std = {
            "low": 0.1,    # High noise for low fidelity
            "medium": 0.01, # Moderate noise
            "high": 0.001  # Low noise
        }[fidelity]

    def __call__(self, x: torch.Tensor) -> torch.Tensor:          # (...,1) → (...)
        x = x.squeeze(-1)
        f = (6.0 * x - 2.0) ** 2 * torch.sin(12.0 * x - 4.0)
        # Add Gaussian noise based on fidelity
        if self.noise_std > 0:
            noise = torch.randn_like(f) * self.noise_std
            f = f + noise
        return f
