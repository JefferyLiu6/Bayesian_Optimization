...
def run_one_bo_loop(obj, acq_name, iters, n_init, kernel_type, train_x=None, train_y=None, output_dir=None, run=None):
    """Run a single BO loop, optionally starting with provided points, and plot for each iteration."""
    d = obj.bounds.size(1)
    ...
    for i in tqdm(range(iters), desc="BO Iteration"):
        ...
        plot_single_run(min_f_values, run, i, output_dir, f"{acq_name}_{obj.fidelity}")
        plot_fidelity_function(obj, train_x, train_y, model, likelihood, obj.fidelity, output_dir, run, i)
    ...
    for run in range(args.runs):
        ...
        for fidelity in ["low", "medium", "high"]:
            ...
            min_f_values, train_x, train_y, model, likelihood = run_one_bo_loop(
                obj, args.policy, iters, n_init, args.kernel, train_x, train_y, out_dir, run
            )
